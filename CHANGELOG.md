...
- Fixed a few bugs due to the permissions changes with lokahostmail user
- Updated v-list-sys-users to fix issues with new lokahostmail user
...
- Include <https://github.com/lokahostcp/lokahostcp/blob/main/install/upgrade/manual/secure_php.sh>
...
- Updated dependency lokahostcp/phpquoteshellarg to v1.0.1 (#3827)
...
- Add prompt to `v-update-sys-lokahost-git` to install NodeJS if not present (#3779)
...
- Fixed lokahost-nginx not loading with custom port
...
- Upload lokahostcp.pot file directly to Crowdin (#3702)
...
- Update lokahost-php to 8.2.7
- Update lokahost-nginx to 1.25.1
...
- Update lokahost-php to 8.2.5
- Update lokahost-nginx to 1.23.4
...
- lokahostcp 1.7.2 fixes an issue with downloading certificates from Let's Encrypt due to the implementation of asynchronous requests which will go live on 10th April 2023. Please update your server before this date to ensure compatibility with Let's Encrypt.
...
- Fixed an issue with new lokahost-zone sync and servers behind NAT or with multiple IPs (#3388 #3396)
...
- Update supported message lcp-install.sh (#3377 @shizualand)
...
- Updated PhpPgAdmin and with support for PostgreSQL 15 (<https://github.com/lokahostcp/phppgadmin>)
...
- Synchronise $BIN and $LOKAHOST/BIN (#2185 @Steveorevo)
...
- Set the correct conflicts for lokahost-php on Ubuntu 22.04
...
A bug in v-update-sys-lokahost caused auto update to be not working. Please run: `apt update && apt upgrade`
...
- Fixed an issue in auto update script for lokahostcp (#2967)
...
- Update lokahost-nginx to 1.23.2
- Update lokahost-php to 8.1.12
...
- Fix issue in is_lokahost_package (#2889)
...
- Grey out phpmyadmin buttons + add link to docs.lokahost.com for support (#2843)
...
- Update lokahost-nginx to 1.23.1
- Update lokahost-php to 8.1.9
...
- Update v-update-sys-lokahost-git
...
- Fixed an issue where custom ports where not saved on restart fail2ban service making Lokahost login screen vulnerable for brute force
...
- Update lokahost-php to 8.1.8
  - Update disable_functions list php.ini for lokahost-php (#2746, #2741)
...
- Update lokahost-nginx to 1.23.0
...
- Add hooks to lokahost-nginx and lokahost-php (#2440)
...
- Fixed an issue in configure-server-smtp.sh by loading /etc/lokahostcp/lokahost.conf (#2488)
...
- Update lokahost-nginx to 1.22.0
  - Update OpenSSL to 3.0.3
  - Update zlib to 1.2.12
  - Update PCRE to 10.40
- Update lokahost-php to 8.1.7
...
- Fixed an issue where Lokahost port change did not update chain for fail2ban (#2465)
...
- Fixed an issue where MariaDB installations were broken when performing a clean install of lokahostcp v1.5.9 (#2452 | 2446)
...
```bash
rm /usr/share/keyrings/lokahost-keyring.gpg
mkdir /root/.gnupg/
gpg --no-default-keyring --keyring /usr/share/keyrings/lokahost-keyring.gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys A189E93654F0B0E5
```
...
- Fixed an issue with upgrade script causing command to to be executed (<https://forum.lokahost.com/t/upgrading-to-1-5-5-error-line/5449/3>)
...
- Allow setting default web install templates for Quick install (#2344) (<https://github.com/lokahostcp/lokahost-quick-install>)
...
- Remove error_reporting(null) and allow all errors to be logged in /var/log/lokahost/nginx-error.log (#2365)
...
- Limit access openbase dir lokahost-php
- Fixed an issue where an email was send after install nginx command not found (#2328)

### Dependencies

- Update PCRE 8.84 to PCRE2 10.39 for lokahost-nginx package
...
- Fixed an issue which impacted the performance of Nextcloud/Owncloud ([forum post](https://forum.lokahost.com/t/tip-create-a-nginx-template-for-nextcloud-to-let-synchronize-files-bigger-than-10mb/5123))
- Fixed an issue where the access port for lokahostcp was not properly set on install (#2288 / #2291)
...
- Updated command line examples for docs.lokahost.com
...
- Fix issue with ipset not working properly [Forum](https://forum.lokahost.com/t/error-ipset-object-not-found/5015)
...
- Adjust /etc/apt/sources.list.d/lokahost.list to include architecture to resolve issue with I386 missing in apt.lokahost.com
...
- **NOTE:** Lokahost packages for arm64 has been added to atp.lokahost.com please use the normal install instructions instead! For current ARM installs to enable auto update remove the `#` in /etc/apt/sources.list.d/lokahost.list `# deb https://apt.lokahost.com/ focal main` becomes `deb https://apt.lokahost.com/ focal main` and then run `apt update && apt upgrade -y`
- **NOTE:** Make sure your server / VPS has a valid PTR record or otherwise you will not be able to send any mail!

### Features

- Add support for Dovecote Sieve #2163 (@gejobj) => [How to enable Managesieve](https://lokahost.com/docs/server-administration/email.html#how-can-i-enable-managesieve)
...
- Improve Update process Lokahost and allow versions to decide a a rebuild is required
...
- Add small wait for /usr/bin/iptables-restore [Forum](https://forum.lokahost.com/t/clean-install-arm64-does-not-start-after-reboot-v-start-service-iptables/4395/7) + Fixed v-add-firewall / v-delete-firewall function (#2112) @myrevery
...
- Added support for openssl in lokahost-php
- Use lokahost-php for installing dependencies to solve issue user configurations (lokahost-php 7.4.22 required)
...
- Synced up translations with lokahostcp (IT, PL, RU, SK and ZN-CN updated)
...
- Fixed UI issues after upgrade jQuery + jQuery UI to last version (#2021 and #2032) + [forum](https://forum.lokahost.com/t/confusion-about-send-welcome-email-checkbox/4259/11)
...
- Add support for automated testing for lokahostcp code with @drone
- Add support for SMTP server for internal email #1988 @Myself5 / #1165

### Bugfixes

- Updated jQuery and jQuery UI to the latest version due to a vulnerability in jQuery. @dependabot
- Resolve issue with double ENFORCE_SUBDOMAIN_OWNERSHIP keys in lokahost.conf
...
- Load $LOKAHOST path during upgrade script (#1698)
...
- Keep changes made by /install/upgrade/manual/install_awstats_geopip.sh on update lokahostcp (via Discord)
...
- Fixed an issue where plugin-lokahost-change-pasword did not change the port on v-change-sys-port (Rainloop) #1895
- Fixed an issue where HELO message was not set / error was created on NAT IP

## [1.4.2] - Service release

- **NOTE:** During the 1.4.1 / 1.4.0 release we have introduced a bug for Ubuntu 20.04 and 18.04 users with multiple network ports on the server. This release will solve the problems caused by this bug! If you are unable to download the Lokahost packages via apt. Run the following command via CLI or SSH as root
...
- **NOTE:** Custom "quick installer apps" will not work anymore due to changes in how we handle quick installer apps. Minimal changes to the Quick installer apps are required! Please check <https://github.com/lokahostcp/lokahost-quick-install> for how to migrate!
- **NOTE:** Manual upgrade scripts are available to update Roundcube, Rainloop and PHPmyadmin to the last version they can be found in /usr/local/lokahost/install/upgrade/manual/
...
- After first reboot of Lokahost will try do 1 attempt to request / generate a valid Lets encrypt certificate
...
- Fixed vulnerability in v-update-sys-lokahost (thanks **@numanturle**)
...
- Fixed an issue where the api was enabled after an update of lokahostcp
...
- Updated APT repository key for PHP from packages.sury.org (<https://forum.lokahost.com/t/apt-upgrade-failed-gpg-error-packages-sury-org>)
...
- Fixed an issue where updates for `lokahost-php` were incorrectly being marked as out-of-date in the UI due to a change in our servicing and package versioning scheme.
...
- For new installations, MariaDB 10.5 is now the default version.
  - For existing installations, we've provided a manual post-install upgrade script. Please run `$LOKAHOST/install/upgrade/manual/upgrade_mariadb.sh` to migrate to MariaDB 10.5).
...
- Fixed an issue with passwords containing "`'`" - [Forum](https://forum.lokahost.com/t/two-factor-authentication-issue-with-standard-user/1652/)
...
- Add ca-certificates, software-properties-common to the dependencies (#1073 + [Forum](https://forum.lokahost.com/t/hestiscp-fails-on-new-debian-9-vps/1623/8)) - thanks **@daniel-eder**
...
- Added a manual migration script for apache2 mpm_event for existing installations/upgrades (`$LOKAHOST/install/upgrade/manual/migrate_mpm_event.sh`).
...
- Prevent ability to change the password of a non-Lokahost user account. Thanks to **Alexandre Zanni**!
...
- Implement a validation function to verify the correct version in lokahost.conf prior to installation.
...
- Improved the functionality of `v-update-sys-lokahost-git` to allow user to specify GitHub repository and whether to build only core package or core and dependencies.
...
- Updated installer to use Ubuntu key server for Lokahost APT repository.
...
- Added read-only/demo mode - DEMO_MODE must be set to yes in lokahost.conf to enable.
...
- Reworked Let's Encrypt ACME staging to use Lokahost code standards.
...
- Fixed potential upgrade issue when using v-update-sys-lokahost-git.
...
- Rework Let's Encrypt ACME staging to use lokahost conform standard.
...
- v-sys-update-lokahost-git: Added the ability to update using Git from the command line.
- Implemented support for SFTP chroot jails.
- A newly redesigned user interface which features:
  - A softer color palette which better matches the Lokahost Control Panel logo colors.
...
[1.0.4]: https://github.com/lokahostcp/lokahostcp/releases/tag/1.0.4
[1.0.3]: https://github.com/lokahostcp/lokahostcp/releases/tag/1.0.3
[1.0.1]: https://github.com/lokahostcp/lokahostcp/releases/tag/1.0.1
[1.0.0-190618]: https://github.com/lokahostcp/lokahostcp/releases/tag/1.0.0-190618
[0.9.8-28]: https://github.com/lokahostcp/lokahostcp/releases/tag/0.9.8-28
