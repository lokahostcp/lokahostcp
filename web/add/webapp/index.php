...
if (!empty($_GET["app"])) {
	...
	$lokahostcp = new \Lokahostcp\System\LokahostcpApp();
	$app_installer_class = "\Lokahostcp\WebApp\Installers\\" . $app . "\\" . $app . "Setup";
	if (class_exists($app_installer_class)) {
		try {
			$app_installer = new $app_installer_class($v_domain, $lokahostcp);
	...
	foreach ($appInstallers as $app) {
		...
		if (
			...
			if ($matches[1] != "Resources") {
				$app_installer_class =
					"\Lokahostcp\WebApp\Installers\\" . $matches[1] . "\\" . $matches[1] . "Setup";
				$app_installer = new $app_installer_class($v_domain, $lokahostcp);
