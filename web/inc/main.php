...
// Load Lokahostcp Config directly
load_lokahostcp_config();
...
function top_panel($user, $TAB) {
	$command = LOKAHOSTCP_CMD . "v-list-user " . $user . " 'json'";
...
function load_lokahostcp_config() {
...
function backendtpl_with_webdomains() {
	exec(LOKAHOSTCP_CMD . "v-list-users json", $output, $return_var);
	...
	foreach ($users as $user => $user_details) {
		exec(
			LOKAHOSTCP_CMD . "v-list-web-domains " . quoteshellarg($user) . " json",
			$output,
			$return_var,
		);
