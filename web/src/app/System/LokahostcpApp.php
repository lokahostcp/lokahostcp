...
class LokahostcpApp {
	/** @var string[] */
	public $errors;
	protected const TMPDIR_DOWNLOADS = "/tmp/lokahostcp-webapp";
	...
	public function run(string $cmd, $args, &$cmd_result = null): bool {
		$cli_script = realpath(LOKAHOSTCP_DIR_BIN . $cmd);
		if (!str_starts_with((string) $cli_script, LOKAHOSTCP_DIR_BIN)) {
			$errstr = "$cmd is trying to traverse outside of " . LOKAHOSTCP_DIR_BIN;
	...
	public function installComposer($version) {
		...
		if (!file_exists($install_folder)) {
			exec(LOKAHOSTCP_CMD . "v-rebuild-user " . $this->user(), $output, $return_code);
			if ($return_code !== 0) {
