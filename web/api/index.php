 ...
 * @return void
 */
function api_error($exit_code, $message, $lcp_return, bool $add_log = false, $user = "system") {
	$message = trim(is_array($message) ? implode("\n", $message) : $message);

	// Add log
	if ($add_log) {
		$v_real_user_ip = get_real_user_ip();
		lcp_add_history_log("[$v_real_user_ip] $message", "API", "Error", $user);
	}
...
function api_legacy(array $request_data) {
	...
	if ($settings["config"]["API"] != "yes") {
		...
		api_error(E_DISABLED, "Error: API Disabled", $lcp_return);
	}
	...
	if ($settings["config"]["API_ALLOWED_IP"] != "allow-all") {
		...
		if (!in_array(get_real_user_ip(), $ip_list)) {
			api_error(E_FORBIDDEN, "Error: IP is not allowed to connect with API", $lcp_return);
		}
	}
	...
	if (empty($request_data["hash"])) {
		...
		if ($request_data["user"] != "$root_user") {
			api_error(E_FORBIDDEN, "Error: authentication failed", $lcp_return);
		}
	...
	$lcp_cmd = trim($request_data["cmd"] ?? "");
	$lcp_cmd_args = [];
	for ($i = 1; $i <= 13; $i++) {
		if (isset($request_data["arg{$i}"])) {
			$lcp_cmd_args["arg{$i}"] = trim($request_data["arg{$i}"]);
		}
	}

	if (empty($lcp_cmd)) {
		api_error(E_INVALID, "Command not provided", $lcp_return);
	} elseif (!preg_match('/^[a-zA-Z0-9_-]+$/', $lcp_cmd)) {
		api_error(E_INVALID, "$lcp_cmd command invalid", $lcp_return);
	}

	// Check command
	if ($lcp_cmd == "v-make-tmp-file") {
		// Used in DNS Cluster
		$fp = fopen("/tmp/" . basename(escapeshellcmd($lcp_cmd_args["arg2"])), "w");
		fwrite($fp, $lcp_cmd_args["arg1"] . "\n");
		fclose($fp);
		$return_var = 0;
	} else {
		// Prepare command
	...
	if (!empty($lcp_return) && $lcp_return == "code") {
		echo $cmd_exit_code;
	} else {
...
function api_connection(array $request_data) {
	...
	if ($api_status == 0) {
		...
		api_error(E_DISABLED, "API has been disabled", $lcp_return);
	}
	...
	if ($settings["config"]["API_ALLOWED_IP"] != "allow-all") {
		...
		if (!in_array($v_real_user_ip, $ip_list) && !in_array("0.0.0.0", $ip_list)) {
			api_error(E_FORBIDDEN, "IP is not allowed to connect with API", $lcp_return);
		}
	}

	// Get POST Params
	$lcp_access_key_id = trim($request_data["access_key"] ?? "");
	$lcp_secret_access_key = trim($request_data["secret_key"] ?? "");
	$lcp_cmd = trim($request_data["cmd"] ?? "");
	$lcp_cmd_args = [];
	for ($i = 1; $i <= 13; $i++) {
		if (isset($request_data["arg{$i}"])) {
	...
	if (empty($lcp_access_key_id) || empty($lcp_secret_access_key)) {
		api_error(E_PASSWORD, "Authentication failed", $lcp_return);
	}
	...
	exec(
		LOKAHOST_CMD .
			...
			" " .
			quoteshellarg($lcp_secret_access_key) .
			" " .
			quoteshellarg($lcp_cmd) .
			" " .
	...
	if ($return_var > 0) {
		//api_error($return_var, "Key $lcp_access_key_id - authentication failed", $lcp_return);
		api_error($return_var, $output, $lcp_return);
	}
	...
	if ($key_user != "admin" && $api_status < 2) {
		api_error(E_API_DISABLED, "API has been disabled", $lcp_return);
	}
	...
	if (
		$key_user != "admin" &&
		$user_arg_position > 0 &&
		$lcp_cmd_args["arg{$user_arg_position}"] != $key_user
	) {
		api_error(
			E_FORBIDDEN,
			"Key $lcp_access_key_id - the \"user\" argument doesn\'t match the key\'s user",
			$lcp_return,
		);
	}
	...
	if ($lcp_cmd == "v-make-tmp-file") {
		$fp = fopen("/tmp/" . basename($lcp_cmd_args["arg2"]), "w");
		fwrite($fp, $lcp_cmd_args["arg1"] . "\n");
		fclose($fp);
		$cmd_exit_code = 0;
	} else {
