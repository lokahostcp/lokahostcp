...
exec(LOKAHOSTCP_CMD . "v-get-sys-timezones json", $output, $return_var);
...
exec(LOKAHOSTCP_CMD . "v-list-remote-dns-hosts json", $output, $return_var);
...
// List Database hosts
exec(LOKAHOSTCP_CMD . "v-list-database-hosts json", $output, $return_var);
...
foreach ($backup_types as $backup_type) {
	if ($backup_type == "local") {
		...
		exec(
			LOKAHOSTCP_CMD . "v-list-backup-host " . quoteshellarg($backup_type) . " json",
			$output,
			$return_var,
		);
...
exec(LOKAHOSTCP_CMD . "v-list-sys-lokahostcp-ssl json", $output, $return_var);
$ssl_str = json_decode(implode("", $output), true);
unset($output);
$v_ssl_crt = $ssl_str["LOKAHOSTCP"]["CRT"];
$v_ssl_key = $ssl_str["LOKAHOSTCP"]["KEY"];
$v_ssl_ca = $ssl_str["LOKAHOSTCP"]["CA"];
$v_ssl_subject = $ssl_str["LOKAHOSTCP"]["SUBJECT"];
$v_ssl_aliases = $ssl_str["LOKAHOSTCP"]["ALIASES"];
$v_ssl_not_before = $ssl_str["LOKAHOSTCP"]["NOT_BEFORE"];
$v_ssl_not_after = $ssl_str["LOKAHOSTCP"]["NOT_AFTER"];
$v_ssl_signature = $ssl_str["LOKAHOSTCP"]["SIGNATURE"];
$v_ssl_pub_key = $ssl_str["LOKAHOSTCP"]["PUB_KEY"];
$v_ssl_issuer = $ssl_str["LOKAHOSTCP"]["ISSUER"];
...
if (!empty($_POST["save"])) {
	...
	if (!empty($_POST["v_hostname"]) && $v_hostname != $_POST["v_hostname"]) {
		exec(
			LOKAHOSTCP_CMD . "v-change-sys-hostname " . quoteshellarg($_POST["v_hostname"]),
			$output,
			$return_var,
		);
	...
	if (empty($_SESSION["error_msg"])) {
		if (!empty($_POST["v_timezone"])) {
			if ($v_timezone != $_POST["v_timezone"]) {
				exec(
					LOKAHOSTCP_CMD . "v-change-sys-timezone " . quoteshellarg($_POST["v_timezone"]),
					$output,
					$return_var,
				);
				check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"])) {
		if (!empty($_POST["v_language"]) && $_SESSION["LANGUAGE"] != $_POST["v_language"]) {
			...
			exec(
				LOKAHOSTCP_CMD . "v-change-sys-language " . quoteshellarg($_POST["v_language"]),
				$output,
				$return_var,
			);
			check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"])) {
		if (
			...
			if ($_POST["v_filemanager"] == "true") {
				exec(LOKAHOSTCP_CMD . "v-add-sys-filemanager", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
				...
				exec(LOKAHOSTCP_CMD . "v-delete-sys-filemanager", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
	...
	if (empty($_SESSION["error_msg"])) {
		if (
			...
			if ($_POST["v_web_terminal"] == "true") {
				exec(LOKAHOSTCP_CMD . "v-add-sys-web-terminal", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
				...
				exec(LOKAHOSTCP_CMD . "v-delete-sys-web-terminal", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
	...
	if (empty($_SESSION["error_msg"])) {
		if (!empty($_POST["v_phpmyadmin_key"])) {
			if ($_POST["v_phpmyadmin_key"] == "yes" && $_SESSION["PHPMYADMIN_KEY"] == "") {
				exec(LOKAHOSTCP_CMD . "v-add-sys-pma-sso quiet", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
				...
				exec(LOKAHOSTCP_CMD . "v-delete-sys-pma-sso quiet", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
					$_SESSION["PHPMYADMIN_KEY"] = "";
	...
	if (empty($_SESSION["error_msg"])) {
		if (!empty($_POST["v_quota"]) && $_SESSION["DISK_QUOTA"] != $_POST["v_quota"]) {
			if ($_POST["v_quota"] == "yes") {
				exec(LOKAHOSTCP_CMD . "v-add-sys-quota", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
				...
				exec(LOKAHOSTCP_CMD . "v-delete-sys-quota", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
	...
	if (empty($_SESSION["error_msg"])) {
		...
		if (!empty($_POST["v_firewall"]) && $v_firewall != $_POST["v_firewall"]) {
			if ($_POST["v_firewall"] == "yes") {
				exec(LOKAHOSTCP_CMD . "v-add-sys-firewall", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
				...
				exec(LOKAHOSTCP_CMD . "v-delete-sys-firewall", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
	...
	if (empty($_SESSION["error_msg"])) {
		...
		if (!isset($_POST["v_smtp_relay"]) && $v_smtp_relay == true) {
			...
			exec(LOKAHOSTCP_CMD . "v-delete-sys-smtp-relay", $output, $return_var);
			check_return_code($return_var, $output);
			unset($output);
		}
	}
	...
	if (empty($_SESSION["error_msg"])) {
		if ($_POST["v_backup"] == "no" && $v_backup == "yes") {
			exec(LOKAHOSTCP_CMD . "v-delete-backup-host local", $output, $return_var);
			check_return_code($return_var, $output);
			unset($output);
			if (empty($_SESSION["error_msg"])) {
				$v_backup = "no";
	...
	if (empty($_SESSION["error_msg"])) {
		if ($_POST["v_backup"] == "yes" && $v_backup != "yes") {
			exec(LOKAHOSTCP_CMD . "v-add-backup-host local", $output, $return_var);
			check_return_code($return_var, $output);
			unset($output);
			if (empty($_SESSION["error_msg"])) {
				$v_backup = "yes";
	...
	if (empty($_SESSION["error_msg"])) {
		...
		if ($_POST["v_backup_dir"] != $v_backup_dir) {
			/*
				See #1655
				exec (LOKAHOSTCP_CMD."v-change-sys-config-value BACKUP ".quoteshellarg($_POST['v_backup_dir']), $output, $return_var);
				check_return_code($return_var,$output);
				unset($output);
				*/
			if (empty($_SESSION["error_msg"])) {
	...
	if (empty($_SESSION["error_msg"])) {
		if (
			...
			exec(
				LOKAHOSTCP_CMD . "v-delete-backup-host " . quoteshellarg($v_backup_type),
				$output,
				$return_var,
			);
			unset($output);
	...
	if (
		...
		if (empty($_SESSION["error_msg"])) {
			if ($_POST["v_api"] == "no" && $_POST["v_api_system"] === 0) {
				exec(LOKAHOSTCP_CMD . "v-change-sys-api 'disable'", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
			}
			if (
				...
				exec(LOKAHOSTCP_CMD . "v-change-sys-api 'enable'", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
			}
		}
		...
		if (empty($_SESSION["error_msg"])) {
			if ($_POST["v_api"] != $_SESSION["API"]) {
				...
				exec(
					LOKAHOSTCP_CMD . "v-change-sys-config-value API " . quoteshellarg($api_status),
					$output,
					$return_var,
				);
				check_return_code($return_var, $output);
	...
	if (!empty($_POST["v_ssl_crt"]) && empty($_SESSION["error_msg"])) {
		if (
			...
			exec(LOKAHOSTCP_CMD . "v-change-sys-lokahostcp-ssl " . $tmpdir, $output, $return_var);
			check_return_code($return_var, $output);
			unset($output);
