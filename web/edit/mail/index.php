...
if (!empty($_GET["domain"]) && empty($_GET["account"])) {
	...
	exec(LOKAHOSTCP_CMD . "v-list-sys-webmail json", $output, $return_var);
...
if (!empty($_POST["save"]) && !empty($_GET["domain"]) && empty($_GET["account"])) {
	...
	if (!empty($_POST["v_reject"]) && $v_antispam == "yes" && $v_reject != "yes") {
		exec(
			LOKAHOSTCP_CMD . "v-add-mail-domain-reject " . $user . " " . $v_domain . " yes",
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_POST["v_reject"]) && $v_reject == "yes") {
		exec(
			LOKAHOSTCP_CMD . "v-delete-mail-domain-reject " . $user . " " . $v_domain . " yes",
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
