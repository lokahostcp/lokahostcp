...
exec(LOKAHOSTCP_CMD . "v-list-web-domains " . $user . " json", $output, $return_var);
...
exec(
	LOKAHOSTCP_CMD . "v-list-web-domain " . $user . " " . quoteshellarg($v_domain) . " json",
	$output,
	$return_var,
);
...
exec(LOKAHOSTCP_CMD . "v-list-web-stats json", $output, $return_var);
...
if (!empty($_POST["save"])) {
	...
	if ($v_ip != $_POST["v_ip"] && empty($_SESSION["error_msg"])) {
		...
		if ($return_var == 0) {
			exec(
				LOKAHOSTCP_CMD . "v-rebuild-mail-domain " . $user . " " . quoteshellarg($v_domain),
				$output,
				$return_var,
			);
			check_return_code($return_var, $output);
	...
	if (
		...
		exec(
			LOKAHOSTCP_CMD . "v-add-web-domain-ssl-force " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($_POST["v_ssl_hsts"]) && !empty($_POST["v_ssl"]) && empty($_SESSION["error_msg"])) {
		exec(
			LOKAHOSTCP_CMD . "v-add-web-domain-ssl-hsts " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($v_stats) && $_POST["v_stats"] == "none" && empty($_SESSION["error_msg"])) {
		exec(
			LOKAHOSTCP_CMD . "v-delete-web-domain-stats " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($restart_web) && empty($_SESSION["error_msg"])) {
		exec(LOKAHOSTCP_CMD . "v-restart-web", $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
	}
	...
	if (
		...
		exec(LOKAHOSTCP_CMD . "v-restart-proxy", $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
	}
	...
	if (!empty($restart_dns) && empty($_SESSION["error_msg"])) {
		exec(LOKAHOSTCP_CMD . "v-restart-dns", $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
	}
