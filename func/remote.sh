...
#===========================================================================#
#                                                                           #
# Lokahostcp Control Panel - API Function Library                               #
#                                                                           #
#===========================================================================#
...
send_api_cmd() {
	...
	if [ "$DEBUG_MODE" = "yes" ]; then
		...
		echo "$1 $2 $3 $4 $5 $6 $7 $8 $9" >> /var/log/lokahostcp/api_out.log
	fi
	return $answer
}
...
is_dnshost_new() {
	if [ -e "$LOKAHOSTCP/conf/dns-cluster.conf" ]; then
		check_host=$(grep "HOST='$host'" $LOKAHOSTCP/conf/dns-cluster.conf)
...
remote_dns_health_check() {
	...
	for str in $(grep "SUSPENDED='no'" $LOKAHOSTCP/conf/dns-cluster.conf); do
