...
#===========================================================================#
#                                                                           #
# Lokahostcp Control Panel - Rebuild Function Library                           #
#                                                                           #
#===========================================================================#
...
rebuild_user_conf() {
	...
	if [ -x "$LOKAHOSTCP/data/packages/$PACKAGE.sh" ]; then
		$LOKAHOSTCP/data/packages/$PACKAGE.sh "$user" "$CONTACT" "$NAME"
	fi
	...
	# Add a general group for normal users created by Lokahostcp
	if [ -z "$(grep "^lokahostcp-users:" /etc/group)" ]; then
		groupadd --system "lokahostcp-users"
	fi

	# Add membership to lokahostcp-users group to non-admin users
	if [ "$user" = "$ROOT_USER" ]; then
		setfacl -m "g:$ROOT_USER:r-x" "$HOMEDIR/$user"
	else
		usermod -a -G "lokahostcp-users" "$user"
		setfacl -m "u:$user:r-x" "$HOMEDIR/$user"
	fi
	setfacl -m "g:lokahostcp-users:---" "$HOMEDIR/$user"
	...
	# Update disk pipe
	sed -i "/ $user$/d" $LOKAHOSTCP/data/queue/disk.pipe
	echo "$BIN/v-update-user-disk $user" >> $LOKAHOSTCP/data/queue/disk.pipe
	...
	if [ -n "$WEB_SYSTEM" ] && [ "$WEB_SYSTEM" != 'no' ]; then
		...
		if [ "$(grep -w $user $LOKAHOSTCP/data/queue/traffic.pipe)" ]; then
			echo "$BIN/v-update-web-domains-traff $user" \
				>> $LOKAHOSTCP/data/queue/traffic.pipe
		fi
		echo "$BIN/v-update-web-domains-disk $user" \
			>> $LOKAHOSTCP/data/queue/disk.pipe
	...
	if [ -n "$DB_SYSTEM" ] && [ "$DB_SYSTEM" != 'no' ]; then
		...
		echo "$BIN/v-update-databases-disk $user" >> $LOKAHOSTCP/data/queue/disk.pipe
...
rebuild_web_domain_conf() {
	...
	if [ -n "$STATS" ]; then
		...
		check_webstats=$(grep "$webstats" $LOKAHOSTCP/data/queue/webstats.pipe)
		if [ -z "$check_webstats" ]; then
			echo "$webstats" >> $LOKAHOSTCP/data/queue/webstats.pipe
		fi
	...
	# domain folder permissions: DOMAINDIR_WRITABLE: default-val:no source:lokahostcp.conf
	DOMAINDIR_MODE=551
...
rebuild_pgsql_database() {
	...
	host_str=$(grep "HOST='$HOST'" $LOKAHOSTCP/conf/pgsql.conf)
...
import_mysql_database() {
	...
	host_str=$(grep "HOST='$HOST'" $LOKAHOSTCP/conf/mysql.conf)
	parse_object_kv_list "$host_str"
