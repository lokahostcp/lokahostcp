...
mysql_connect() {
	unset PORT
	host_str=$(grep "HOST='$1'" $LOKAHOSTCP/conf/mysql.conf)
	...
	mycnf="$LOKAHOSTCP/conf/.mysql.$HOST"
	if [ ! -e "$mycnf" ]; then
		echo "[client]" > $mycnf
...
psql_connect() {
	unset PORT
	host_str=$(grep "HOST='$1'" $LOKAHOSTCP/conf/pgsql.conf)
	parse_object_kv_list "$host_str"
...
get_next_dbhost() {
	if [ -z "$host" ] || [ "$host" == 'default' ]; then
		IFS=$'\n'
		host='EMPTY_DB_HOST'
		config="$LOKAHOSTCP/conf/$type.conf"
...
is_charset_valid() {
	host_str=$(grep "HOST='$host'" $LOKAHOSTCP/conf/$type.conf)
	parse_object_kv_list "$host_str"
...
increase_dbhost_values() {
	...
	sed -i "s/$old_dbbases/$new_dbbases/g" $LOKAHOSTCP/conf/$type.conf
	sed -i "s/$old_users/$new_users/g" $LOKAHOSTCP/conf/$type.conf
}
...
is_dbhost_new() {
	if [ -e "$LOKAHOSTCP/conf/$type.conf" ]; then
		check_host=$(grep "HOST='$host'" $LOKAHOSTCP/conf/$type.conf)
		if [ "$check_host" ]; then
