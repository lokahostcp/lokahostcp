...
#===========================================================================#
#                                                                           #
# Lokahostcp Control Panel - Backup Function Library                            #
#                                                                           #
#===========================================================================#
...
local_backup() {
	...
	if [ "$disk_usage" -ge "$BACKUP_DISK_LIMIT" ]; then
		...
		sed -i "/ $user /d" $LOKAHOSTCP/data/queue/backup.pipe
...
ftp_backup() {
	# Checking config
	if [ ! -e "$LOKAHOSTCP/conf/ftp.backup.conf" ]; then
	...
	# Parse config
	source_conf "$LOKAHOSTCP/conf/ftp.backup.conf"

	# Set default port
	if [ -z "$(grep 'PORT=' $LOKAHOSTCP/conf/ftp.backup.conf)" ]; then
		PORT='21'
	fi
...
sftp_download() {
	source_conf "$LOKAHOSTCP/conf/sftp.backup.conf"
	if [ -z "$PORT" ]; then
		PORT='22'
	fi
	cd $BACKUP
...
sftp_backup() {
	# Checking config
	if [ ! -e "$LOKAHOSTCP/conf/sftp.backup.conf" ]; then
	...
	# Set default port
	if [ -z "$(grep 'PORT=' $LOKAHOSTCP/conf/sftp.backup.conf)" ]; then
		PORT='22'
	fi
...
b2_backup() {
	...
	source_conf "$LOKAHOSTCP/conf/b2.backup.conf"
...
rclone_backup() {
	...
	source_conf "$LOKAHOSTCP/conf/rclone.backup.conf"
