...
#===========================================================================#
#                                                                           #
# Lokahost Control Panel - Backup Function Library                            #
#                                                                           #
#===========================================================================#
...
local_backup() {
	...
	if [ "$disk_usage" -ge "$BACKUP_DISK_LIMIT" ]; then
		...
		sed -i "/ $user /d" $LOKAHOST/data/queue/backup.pipe
...
ftp_backup() {
	# Checking config
	if [ ! -e "$LOKAHOST/conf/ftp.backup.conf" ]; then
	...
	# Parse config
	source_conf "$LOKAHOST/conf/ftp.backup.conf"

	# Set default port
	if [ -z "$(grep 'PORT=' $LOKAHOST/conf/ftp.backup.conf)" ]; then
		PORT='21'
	fi
...
sftp_download() {
	source_conf "$LOKAHOST/conf/sftp.backup.conf"
	if [ -z "$PORT" ]; then
		PORT='22'
	fi
	cd $BACKUP
...
sftp_backup() {
	# Checking config
	if [ ! -e "$LOKAHOST/conf/sftp.backup.conf" ]; then
	...
	# Set default port
	if [ -z "$(grep 'PORT=' $LOKAHOST/conf/sftp.backup.conf)" ]; then
		PORT='22'
	fi
...
b2_backup() {
	...
	source_conf "$LOKAHOST/conf/b2.backup.conf"
...
rclone_backup() {
	...
	source_conf "$LOKAHOST/conf/rclone.backup.conf"
