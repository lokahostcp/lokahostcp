...
#===========================================================================#
#                                                                           #
# Lokahostcp Control Panel - IP/Network Function Library                        #
#                                                                           #
#===========================================================================#
...
# Check ip ownership
is_ip_owner() {
	owner=$(grep 'OWNER=' $LOKAHOSTCP/data/ips/$ip | cut -f 2 -d \')
	if [ "$owner" != "$user" ]; then
...
is_ip_free() {
	if [ -e "$LOKAHOSTCP/data/ips/$ip" ]; then
...
is_ip_key_empty() {
	key="$1"
	string=$(cat $LOKAHOSTCP/data/ips/$ip)
	eval $string
	eval value="$key"
...
# Get ip name
get_ip_alias() {
	ip_name=$(grep "NAME=" $LOKAHOSTCP/data/ips/$local_ip | cut -f 2 -d \')
	if [ -n "$ip_name" ]; then
		echo "${1//./-}.$ip_name"
	fi
}
...
increase_ip_value() {
	...
	usr_key='U_SYS_USERS'
	current_web=$(grep "$web_key=" $LOKAHOSTCP/data/ips/$sip | cut -f 2 -d \')
	current_usr=$(grep "$usr_key=" $LOKAHOSTCP/data/ips/$sip | cut -f 2 -d \')
...
# Get real ip address
get_real_ip() {
	if [ -e "$LOKAHOSTCP/data/ips/$1" ]; then
		echo "$1"
	else
		nat=$(grep -H "^NAT='$1'" $LOKAHOSTCP/data/ips/* | head -n1)
		if [ -n "$nat" ]; then
...
# Get user ips
get_user_ips() {
	dedicated=$(grep -H "OWNER='$user'" $LOKAHOSTCP/data/ips/*)
	dedicated=$(echo "$dedicated" | cut -f 1 -d : | sed 's=.*/==' | grep -E ${REGEX_IPV4})
	shared=$(grep -H -A1 "OWNER='$ROOT_USER'" $LOKAHOSTCP/data/ips/* | grep shared)
...
get_user_ip() {
	...
	local_ip=$ip
	nat=$(grep "^NAT" $LOKAHOSTCP/data/ips/$ip | cut -f 2 -d \')
	if [ -n "$nat" ]; then
		ip=$nat
	fi
}
...
is_ip_valid() {
	local_ip="$1"
	if [ ! -e "$LOKAHOSTCP/data/ips/$1" ]; then
		nat=$(grep -H "^NAT='$1'" $LOKAHOSTCP/data/ips/*)
		if [ -z "$nat" ]; then
	...
	if [ -n "$2" ]; then
		if [ -z "$nat" ]; then
			ip_data=$(cat $LOKAHOSTCP/data/ips/$1)
		else
			ip_data=$(cat $LOKAHOSTCP/data/ips/$nat)
		fi
