...
#
# Script for preparing lxd environment and building Lokahostcp packages for all supported distros
...
function setup_container() {
	...
	lxc config device add ${containername} debdir disk path=/opt/lokahostcp source=${__DIR__}/build/${containername}
}
...
for osdef in "${oslist[@]}"; do
	...
	for osver in $osversions; do
		...
		containername="lcp-${osname}-${osver/\./}"
		container_ip=""
		...
		if [ "$cmd" = '--background' ]; then
			...
			lxc exec $containername -- /opt/lokahostcp/lxd_compile.sh $branch > /dev/null 2>&1 &
		else
			lxc exec $containername -- /opt/lokahostcp/lxd_compile.sh $branch
		fi
