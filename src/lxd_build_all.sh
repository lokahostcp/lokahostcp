...
#
# Script for preparing lxd environment and building Lokahost packages for all supported distros
...
function setup_container() {
	...
	lxc config device add ${containername} debdir disk path=/opt/lokahost source=${__DIR__}/build/${containername}
}
...
for osdef in "${oslist[@]}"; do
	...
	for osver in $osversions; do
		...
		if [ "$cmd" = '--background' ]; then
			...
			lxc exec $containername -- /opt/lokahost/lxd_compile.sh $branch > /dev/null 2>&1 &
		else
			lxc exec $containername -- /opt/lokahost/lxd_compile.sh $branch
		fi
