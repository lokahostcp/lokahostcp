...
/* Lokahost way to enable support for SSO to PHPmyAdmin */
...
define("API_LOKAHOST_PORT", "%API_LOKAHOST_PORT%");
...
class Lokahost_API {
	...
	public function __construct() {
		$this->hostname = "https://" . API_HOST_NAME . ":" . API_LOKAHOST_PORT . "/api/";
		$this->key = API_KEY;
...
if (!empty($_GET)) {
	if (isset($_GET["logout"])) {
		...
		if (isset($_GET["user"]) && isset($_GET["lokahost_token"])) {
			$database = $_GET["database"];
			$user = $_GET["user"];
			$host = "localhost";
			$token = $_GET["lokahost_token"];
