...
/* Lokahostcp way to enable support for SSO to PHPmyAdmin */
...
define("API_LOKAHOSTCP_PORT", "%API_LOKAHOSTCP_PORT%");
define("API_KEY", "%API_KEY%");

class Lokahostcp_API {
	/** @var string */
	public $hostname;
	/** @var string */
	public $key;
	...
	public function __construct() {
		$this->hostname = "https://" . API_HOST_NAME . ":" . API_LOKAHOSTCP_PORT . "/api/";
		$this->key = API_KEY;
...
$api = new Lokahostcp_API();
if (!empty($_GET)) {
	if (isset($_GET["logout"])) {
		$api->delete_temp_user(
			$_SESSION["LOKAHOSTCP_sso_database"],
			$_SESSION["LOKAHOSTCP_sso_user"],
			$_SESSION["PMA_single_signon_user"],
			$_SESSION["LOKAHOSTCP_sso_host"],
		);
		//remove session
		session_invalid();
	} else {
		if (isset($_GET["user"]) && isset($_GET["lokahostcp_token"])) {
			$database = $_GET["database"];
			$user = $_GET["user"];
			$host = "localhost";
			$token = $_GET["lokahostcp_token"];
			...
			if ($time + 60 > time()) {
				...
				if ($data) {
					...
					$_SESSION["LOKAHOSTCP_sso_user"] = $user;
					$_SESSION["LOKAHOSTCP_sso_database"] = $database;
					$_SESSION["LOKAHOSTCP_sso_host"] = $host;
