...
echo "To remove phpMyAdmin you will need use the root password. Password can be found in /usr/local/lokahostcp/conf/mysql.conf"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	if [ "$WEB_SYSTEM" == "apache2" ]; then
		cp -f $LOKAHOSTCP_INSTALL_DIR/pma/apache.conf /etc/phpmyadmin/
	...
	if [ -z "$DB_PMA_ALIAS" ]; then
		echo "DB_PMA_ALIAS='phpmyadmin'" >> $LOKAHOSTCP/conf/lokahostcp.conf
	fi
...
MYSQL_PMA1
	...
	mysql -uroot < $LOKAHOSTCP_INSTALL_DIR/phpmyadmin/create_tables.sql
