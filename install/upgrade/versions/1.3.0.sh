...
# Lokahost Control Panel upgrade script for target version 1.3.0
...
if [ ! -e "/etc/cron.d/lokahost-proc" ]; then
...
# Set LOGIN_STYLE variable in lokahost.conf
echo "[ * ] Updating configuration file: lokahost.conf..."
$BIN/v-change-sys-config-value "LOGIN_STYLE" "default"

# Remove old lanugage files.
if [ -e $LOKAHOST/web/inc/i18n/en.php ]; then
	echo "[ * ] Performing language library clean-up..."
	rm -fr $LOKAHOST/web/inc/i18n
fi
