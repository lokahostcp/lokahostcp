...
# Lokahost Control Panel upgrade script for target version 1.7.0
...
if [ "$FILE_MANAGER" = "true" ]; then
	...
	$LOKAHOST/bin/v-delete-sys-filemanager quiet
...
for package in $packages; do
	if [ -z "$(grep -e 'RATE_LIMIT' $LOKAHOST/data/packages/$package)" ]; then
		echo "RATE_LIMIT='200'" >> $LOKAHOST/data/packages/$package
	fi
done
...
for file in $LOKAHOST/data/templates/web/php-fpm/*; do
	echo "[ * ] Update $file legacy /var/run/ to /run/..."
	sed -i 's|/var/run/|/run/|g' $file
done
...
if [ -f /etc/logrotate.d/httpd-prerotate/awstats ]; then
	echo "[ * ] Update Awstats prerotate to Lokahost update method..."
	# Replace awstatst function
	cp -f $LOKAHOST_INSTALL_DIR/logrotate/httpd-prerotate/awstats /etc/logrotate.d/httpd-prerotate/
fi

if [ "$PHPMYADMIN_KEY" != "" ]; then
	echo "[ * ] Refresh lokahost-sso for PMA..."
