...
# Lokahost Control Panel upgrade script for target version 1.4.13
...
# Remove old files/folders from previous versions of Lokahost Control Panel
if [ -d "$LOKAHOST/web/edit/file/" ]; then
	rm -fr $LOKAHOST/web/edit/file/
fi
if [ -d "$LOKAHOST/web/edit/server/theme/" ]; then
	rm -fr $LOKAHOST/web/edit/server/theme/
fi
