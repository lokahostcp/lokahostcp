...
# Lokahost Control Panel upgrade script for target version 1.1.0
...
if [ ! -z "$IMAP_SYSTEM" ]; then
	...
	if [ -e /etc/nginx/conf.d/webmail.inc ]; then
		cp -f /etc/nginx/conf.d/webmail.inc $LOKAHOST_BACKUP/conf/
...
if [ -e $LOKAHOST/data/users/history.log ]; then
	rm -f $LOKAHOST/data/users/history.log
fi
...
if [ -e "/etc/cron.d/lokahost-sftp" ]; then
	if ! cat /etc/cron.d/lokahost-sftp | grep -q 'root'; then
		echo "@reboot root /usr/local/lokahost/bin/v-add-sys-sftp-jail" > /etc/cron.d/lokahost-sftp
	fi
fi

# Create default writeable folders for all users
echo "[ * ] Updating default writable folders for all users..."
for user in $($LOKAHOST/bin/v-list-sys-users plain); do
	mkdir -p \
		$HOMEDIR/$user/.cache \
		$HOMEDIR/$user/.config \
		$HOMEDIR/$user/.local \
...
if fail2ban-client status sshd > /dev/null 2>&1; then
	...
	if [ -f /etc/fail2ban/jail.d/defaults-debian.conf ]; then
		mkdir -p $LOKAHOST_BACKUP/conf/fail2ban/jail.d
		mv /etc/fail2ban/jail.d/defaults-debian.conf $LOKAHOST_BACKUP/conf/fail2ban/jail.d/
	fi
fi

# Update Office 365/Microsoft 365 DNS template
if [ -e "$LOKAHOST/data/templates/dns/office365.tpl" ]; then
	echo "[ * ] Updating DNS template for Office 365..."
	cp -f $LOKAHOST/install/deb/templates/dns/office365.tpl $LOKAHOST/data/templates/dns/office365.tpl
fi

# Ensure that backup compression level is correctly set
GZIP_LVL_CHECK=$(cat $LOKAHOST/conf/lokahost.conf | grep BACKUP_GZIP)
...
for user in $($LOKAHOST/bin/v-list-users plain | cut -f1); do
	for domain in $($LOKAHOST/bin/v-list-web-domains $user plain | cut -f1); do
		chown root:$user /var/log/$WEB_SYSTEM/domains/$domain.* > /dev/null 2>&1
		for sub_domain in $($LOKAHOST/bin/v-list-web-domain $user $domain plain | cut -f7 | tr ',' '\n'); do
...
if [ "$IMAP_SYSTEM" = "dovecot" ]; then
	...
	if [ -e /etc/dovecot/conf.d/20-pop3.conf ]; then
		cp -f $LOKAHOST/install/deb/dovecot/conf.d/20-pop3.conf /etc/dovecot/conf.d/20-pop3.conf
	fi
	if [ -e /etc/dovecot/conf.d/20-imap.conf ]; then
		cp -f $LOKAHOST/install/deb/dovecot/conf.d/20-imap.conf /etc/dovecot/conf.d/20-imap.conf
	fi
	if [ -e /etc/dovecot/conf.d/90-quota.conf ]; then
		cp -f $LOKAHOST/install/deb/dovecot/conf.d/90-quota.conf /etc/dovecot/conf.d/90-quota.conf
	fi
fi
...
if [ "$num_php_versions" -gt 1 ] && [ -z "$WEB_BACKEND" ]; then
	...
	cp -rf $LOKAHOST/data/templates/web $LOKAHOST_BACKUP/templates/web
	bash $LOKAHOST/install/upgrade/manual/migrate_multiphp.sh > /dev/null 2>&1
fi
