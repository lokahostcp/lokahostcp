...
# Lokahost Control Panel upgrade script for target version 1.5.5
...
for version in $($BIN/v-list-sys-php plain); do
	cp -f $LOKAHOST_INSTALL_DIR/php-fpm/php-fpm.conf /etc/php/$version/fpm/
...
if [ ! -f "/usr/share/keyrings/nginx-keyring.gpg" ]; then
	...
	if [ -f "$apt/lokahost.list" ]; then
		rm $apt/lokahost.list
		echo "   [ * ] Lokahost"
...
if [ ! -f "$LOKAHOST/data/packages/system.pkg" ]; then
	echo "[ * ] Install default system package."
	cp -f $LOKAHOST/install/deb/packages/system.pkg $LOKAHOST/data/packages/system.pkg
fi
