...
# Lokahost Control Panel upgrade script for target version 1.4.8
...
$LOKAHOST/bin/v-add-sys-phpmailer quiet

matches=$(grep -o 'ENFORCE_SUBDOMAIN_OWNERSHIP' $LOKAHOST/conf/lokahost.conf | wc -l)
if [ "$matches" -gt 1 ]; then
	echo "[ * ] Removing double matches ENFORCE_SUBDOMAIN_OWNERSHIP key"
	source $LOKAHOST/conf/lokahost.conf
	sed -i "/ENFORCE_SUBDOMAIN_OWNERSHIP='$ENFORCE_SUBDOMAIN_OWNERSHIP'/d" $LOKAHOST/conf/lokahost.conf
	$LOKAHOST/bin/v-change-sys-config-value "ENFORCE_SUBDOMAIN_OWNERSHIP" "$ENFORCE_SUBDOMAIN_OWNERSHIP"
fi
