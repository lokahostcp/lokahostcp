...
# Lokahost Control Panel upgrade script for target version 1.4.0
...
if [ "$API" = "no" ]; then
	if [ -f "$LOKAHOST/web/api/index.php" ]; then
		echo "[ * ] Disabling API access..."
		$LOKAHOST/bin/v-change-sys-api remove
	fi
fi

# Back up users existing configuration data to $LOKAHOST/conf/defaults/lokahost.conf
if [ ! -f "$LOKAHOST/conf/defaults/lokahost.conf" ]; then
	...
	cp -f $LOKAHOST/conf/lokahost.conf $LOKAHOST/conf/defaults/lokahost.conf
fi
...
if [ "$WEB_SYSTEM" = "nginx" ]; then
	...
	sed -i "s|TPL='drupal6'|TPL='drupal'|g" $LOKAHOST/data/users/*/web.conf
	sed -i "s|TPL='drupal7'|TPL='drupal'|g" $LOKAHOST/data/users/*/web.conf
	sed -i "s|TPL='drupal8'|TPL='drupal'|g" $LOKAHOST/data/users/*/web.conf
	sed -i "s|TPL='codeigniter2'|TPL='codeigniter'|g" $LOKAHOST/data/users/*/web.conf
	sed -i "s|TPL='codeigniter3'|TPL='codeigniter'|g" $LOKAHOST/data/users/*/web.conf
fi

# Remove outdated nginx templates
echo "[ * ] Removing outdated nginx templates..."
rm -rf $LOKAHOST/data/templates/web/nginx/php-fpm/drupal6.*tpl
rm -rf $LOKAHOST/data/templates/web/nginx/php-fpm/drupal7.*tpl
rm -rf $LOKAHOST/data/templates/web/nginx/php-fpm/drupal8.*tpl
rm -rf $LOKAHOST/data/templates/web/nginx/php-fpm/codeigniter2.*tpl
rm -rf $LOKAHOST/data/templates/web/nginx/php-fpm/codeigniter3.*tpl

# Clean up old Lokahost controlled webapps
if [ -d "$LOKAHOST/web/images/webapps/" ]; then
	echo "[ * ] Clean up old web apps code..."
	rm -rf $LOKAHOST/web/images/webapps/
	rm -rf $LOKAHOST/web/src/app/WebApp/Installers/LaravelSetup.php
	rm -rf $LOKAHOST/web/src/app/WebApp/Installers/OpencartSetup.php
	rm -rf $LOKAHOST/web/src/app/WebApp/Installers/PrestashopSetup.php
	rm -rf $LOKAHOST/web/src/app/WebApp/Installers/SymfonySetup.php
	rm -rf $LOKAHOST/web/src/app/WebApp/Installers/WordpressSetup.php
	rm -rf $LOKAHOST/web/src/app/WebApp/Installers/Joomla
fi
...
if [ -d $LOKAHOST/data/packages/ ]; then
	echo "[ * ] Migrating legacy default package for all users..."
	$LOKAHOST/bin/v-rename-user-package default custom > /dev/null 2>&1
	echo "[ * ] Replacing default package..."
	cp -f $LOKAHOST_INSTALL_DIR/packages/default.pkg $LOKAHOST/data/packages/
fi
