...
# Lokahost Control Panel upgrade script for target version 1.6.0
...
if [ "$MAIL_SYSTEM" = "exim4" ]; then
	...
	cp $LOKAHOST_INSTALL_DIR/exim/limit.conf /etc/exim4/limit.conf
	cp $LOKAHOST_INSTALL_DIR/exim/system.filter /etc/exim4/system.filter
...
if [ -z "$(grep v-update-lets $LOKAHOST/data/users/admin/cron.conf)" ]; then
...
cp -rf $LOKAHOST_INSTALL_DIR/api $LOKAHOST/data/
...
if [ "$release" = "22.04" ]; then
	if [ -d "/etc/exim4/" ]; then
		...
		cp -f $LOKAHOST_INSTALL_DIR/exim/exim4.conf.4.94.template /etc/exim4/exim4.conf.template
...
if [ -f "/etc/cron.d/lokahost-sftp" ]; then
	rm /etc/cron.d/lokahost-sftp
	echo "@reboot root sleep 60 && /usr/local/lokahost/bin/v-add-sys-sftp-jail > /dev/null" > /etc/cron.d/lokahost-sftp
fi

ips=$(ls /usr/local/lokahost/data/ips/ | wc -l)
release=$(lsb_release -s -i)
if [ $release = 'Ubuntu' ]; then
	if [ $ips -gt 1 ]; then
		add_upgrade_message "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. See https://github.com/lokahost/lokahost/pull/2612#issuecomment-1135571835 for more info regarding this issue!"
		$LOKAHOST/bin/v-add-user-notification admin "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. <a href='https://github.com/lokahost/lokahost/pull/2612#issuecomment-1135571835'>More info</a>"
	fi
fi
