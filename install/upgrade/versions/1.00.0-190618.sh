...
# Lokahost Control Panel upgrade script for target version 1.00.0-190618
...
if [ -z "$WEBMAIL_ALIAS" ]; then
	...
	$LOKAHOST/bin/v-change-sys-config-value 'WEBMAIL_ALIAS' "webmail"
fi

# Update Apache and Nginx configuration to support new file structure
if [ -f /etc/apache2/apache.conf ]; then
	echo "[ * ] Updating Apache configuration..."
	mv /etc/apache2/apache.conf $LOKAHOST_BACKUP/conf/
	cp -f $LOKAHOST_INSTALL_DIR/apache2/apache.conf /etc/apache2/apache.conf
fi
if [ -f /etc/nginx/nginx.conf ]; then
	echo "[ * ] Updating NGINX configuration..."
	mv /etc/nginx/nginx.conf $LOKAHOST_BACKUP/conf/
	cp -f $LOKAHOST_INSTALL_DIR/nginx/nginx.conf /etc/nginx/nginx.conf
fi
...
if [ -d $LOKAHOST/data/packages/ ]; then
	...
	cp -f $LOKAHOST/data/packages/default.pkg $LOKAHOST_BACKUP/packages/
fi

# Remove old Office 365 template as there is a newer version with an updated name
if [ -f $LOKAHOST/data/templates/dns/o365.tpl ]; then
	rm -f $LOKAHOST/data/templates/dns/o365.tpl
fi

# Back up and remove default index.html if it exists
if [ -f /var/www/html/index.html ]; then
	mv /var/www/html/index.html $LOKAHOST_BACKUP/templates/
fi
...
cp -rf $LOKAHOST_INSTALL_DIR/templates/web/unassigned/* /var/www/html/
cp -rf $LOKAHOST_INSTALL_DIR/templates/web/skel/document_errors/* /var/www/document_errors/
...
# Make sure non-admin users belong to correct Lokahost group
for user in $($BIN/v-list-users plain | cut -f1); do
	if [ "$user" != "admin" ]; then
		...
		for ftp_user in $(cat /etc/passwd | grep -v "^$user:" | grep "^$user.*:$uid:$uid:" | cut -d ":" -f1); do
			usermod -a -G "lokahost-users" "$ftp_user"
		done
	fi
...
for ipaddr in $($BIN/v-list-sys-ips plain | cut -f1); do
	...
	if [ "$WEB_SYSTEM" = "apache2" ]; then
		...
		cat $LOKAHOST_INSTALL_DIR/apache2/unassigned.conf >> $web_conf
		...
		cp -f $LOKAHOST_INSTALL_DIR/nginx/unassigned.inc $web_conf
		sed -i 's/directIP/'$ipaddr'/g' $web_conf
	fi
...
if [ "$php_versions" -gt 1 ]; then
	...
	for v in $($BIN/v-list-sys-php plain); do
		...
		cp -f $LOKAHOST_INSTALL_DIR/php-fpm/dummy.conf /etc/php/$v/fpm/pool.d/
		v1=$(echo "$v" | sed -e 's/[.]//')
...
if [ -d $LOKAHOST/install/ubuntu ]; then
	echo "[ * ] Removing old LokahostCP installation files for Ubuntu..."
	rm -rf $LOKAHOST/install/ubuntu
fi
if [ -d $LOKAHOST/install/debian ]; then
	echo "[ * ] Removing old LokahostCP installation files for Debian..."
	rm -rf $LOKAHOST/install/debian
fi
...
if [ -f /etc/dovecot/conf.d/15-mailboxes.conf ]; then
	mv /etc/dovecot/conf.d/15-mailboxes.conf $LOKAHOST_BACKUP/conf/
fi
if [ -f /etc/dovecot/dovecot.conf ]; then
	# Update Dovecot configuration and restart Dovecot service
	mv /etc/dovecot/dovecot.conf $LOKAHOST_BACKUP/conf/
	cp -f $LOKAHOST_COMMON_DIR/dovecot/dovecot.conf /etc/dovecot/dovecot.conf
...
if [ -f /etc/exim4/exim4.conf.template ]; then
	...
	mv /etc/exim4/exim4.conf.template $LOKAHOST_BACKUP/conf/
	cp -f $LOKAHOST_INSTALL_DIR/exim/exim4.conf.template /etc/exim4/exim4.conf.template
...
if [ -z "$IMAP_SYSTEM" ]; then
	if [ -f /usr/bin/dovecot ]; then
		echo "[ * ] Adding missing IMAP_SYSTEM variable to lokahost.conf..."
		echo "IMAP_SYSTEM = 'dovecot'" >> $LOKAHOST/conf/lokahost.conf
	fi
fi

# Run sftp jail once
$LOKAHOST/bin/v-add-sys-sftp-jail
