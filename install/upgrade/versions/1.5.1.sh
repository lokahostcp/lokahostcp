...
# Lokahost Control Panel upgrade script for target version 1.5.1
...
chmod +x $LOKAHOST/install/deb/

echo "[ * ] Updating lokahost apt configuration..."
sed -i "s|deb https://$RHOST/ $codename main|deb [arch=$ARCH] https://$RHOST/ $codename main|g" /etc/apt/sources.list.d/lokahost.list
