...
# ======================================================== #
#
# Lokahostcp Control Panel Installer for Debian
...
conflicts_pkg="exim4 mariadb-server apache2 nginx lokahostcp postfix"
...
if [ "$exim" = 'yes' ]; then
	...
	if ! version_ge "4.95" "$exim_version"; then
		...
		if ! version_ge "4.93" "$exim_version"; then
			cp -f $LOKAHOSTCP_INSTALL_DIR/exim/exim4.conf.4.94.template /etc/exim4/exim4.conf.template
		else
