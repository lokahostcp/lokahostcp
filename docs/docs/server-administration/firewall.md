...
::: warning
After every edit or update the firewall, Lokahostcp will clear the current iptables unless the rules are added via Lokahostcp and [custom script](#how-can-i-customize-iptables-rules).
:::
...
6. Select the data source by entering one of the following:
   ...
   - Script (with `chmod 755`): `/usr/local/lokahostcp/install/deb/firewall/ipset/blacklist.sh`
   - File: `file:/location/of/file`
   - You can also use one of Lokahostcp’s included sources.
...
Lokahostcp supports setting custom rules, chains or flags, etc. using script.

Script must be here: `/usr/local/lokahostcp/data/firewall/custom.sh`

1. Create custom.sh: `touch /usr/local/lokahostcp/data/firewall/custom.sh`
2. Make it executable: `chmod +x /usr/local/lokahostcp/data/firewall/custom.sh`
