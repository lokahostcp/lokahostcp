...
## Host DNS for your domain on Lokahostcp

Pre-requisites

These steps require that you configure the DNS servers of your domain to use your Lokahostcp servers.
...
1. On your Lokahostcp master, [create a DNS Zone](../user-guide/dns#adding-a-dns-zone) with the **child-ns** template
2. On your domain registrar panel, set the name servers of the domain to the Lokahostcp servers
...
A Master server is where DNS zones are created, and a Slave server recieves the zone via the API. Lokahostcp can be configured as Master <-> Master or Master -> Slave. With a Master <-> Master configuration, each Master is also a Slave, so it could be considered as Master/Slave <-> Master/Slave.
...
### Master <-> Master DNS cluster (Default setup) with the Lokahostcp API

::: warning
This method does not support DNSSEC!
:::

1. Create a new user on each Lokahostcp server that will act as a “Slave”. Make sure it uses the username of "dns-cluster" or has the role `dns-cluster`
...
### Master -> Slave DNS cluster with the Lokahostcp API
...
5. Edit `/usr/local/lokahostcp/conf/lokahostcp.conf`, change `DNS_CLUSTER_SYSTEM='lokahostcp'` to `DNS_CLUSTER_SYSTEM='lokahostcp-zone'`.
...
1. In `/usr/local/lokahostcp/conf/lokahostcp.conf`, change `DNS_CLUSTER_SYSTEM='lokahostcp'` to `DNS_CLUSTER_SYSTEM='lokahostcp-zone'`.
...
::: warning
DNSSEC can’t be used when Lokahostcp Cluster is active as Master <-> Master
:::
...
::: danger
Removing or disabling the private key in Lokahostcp will make the domain inaccessble.
:::
...
```bash
/usr/local/lokahostcp/func/remote.sh: line 43: return: Error:: numeric argument required
