...
2. On your Lokahostcp server run the following command (`mysql` may be replaced by `postgresql`):
...
Make sure the API is enabled and working properly. Lokahostcp’s PhpMyAdmin Single Sign On function connects over the Lokahostcp API.
...
```bash
v-add-database-host mysql db.lokahost.online root mypassword 500
```
...
See `/usr/local/lokahostcp/install/deb/phpmyadmin/pma.sh`
