...
2. On your Lokahost server run the following command (`mysql` may be replaced by `postgresql`):
...
Make sure the API is enabled and working properly. Lokahost’s PhpMyAdmin Single Sign On function connects over the Lokahost API.
...
```bash
v-add-database-host mysql db.lokahost.com root mypassword 500
```
...
See `/usr/local/lokahost/install/deb/phpmyadmin/pma.sh`
