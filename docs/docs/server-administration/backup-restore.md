...
For safety reasons, Lokahost takes into account 2x the user’s disk usage when creating a backup. Therefore, before starting a backup, we check how much disk usage a user has left. If you encounter this error, you can do one of the following to solve the issue:
...
Once the config has been saved you can setup Lokahost with the following command:
...
```bash
v-add-backup-host 'rclone' 'b2' '' '' 'lokahost'
```
