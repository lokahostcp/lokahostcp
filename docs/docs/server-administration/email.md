...
By default, emails generated by Lokahost (Notifications, Forgot password, update logs, etc.) are sent the internal mail. If you want, you can setup mail to be send over a SMTP account.

Execute the following script and follow the instructions:

```bash
bash /usr/local/lokahost/install/upgrade/manual/configure-server-smtp.sh
```
...
No, Cloudflare’s Proxy does not work with email. If you use email hosted on you server, make sure the proxy for the A record `mail.domain.tld` is switched off. Otherwise, you will not be able receive emails. The following records are suggested if you want to use Lokahost as your mail server:
...
During Lokahost’s installation, use the `--sieve` flag. If Lokahost is already installed, there is a an upgrade script provided at the following path: `/usr/local/lokahost/install/upgrade/manual/install_sieve.sh`
...
[See forum topic for more info](https://forum.lokahost.com/t/oracle-cloud-email-as-relay-doesnt-works/11304/19?)
