...
# Add the beta repo to lokahostcp.list
sed -i 's/^/#/' $apt/lokahostcp.list
echo "deb [arch=$ARCH signed-by=/usr/share/keyrings/lokahostcp-beta-keyring.gpg] https://beta-apt.lokahost.online/ $codename main" >> $apt/lokahostcp.list
curl -s "https://beta-apt.lokahost.online/pubkey.gpg" | gpg --dearmor | tee /usr/share/keyrings/lokahostcp-beta-keyring.gpg > /dev/null 2>&1
...
If you want to install a new Lokahostcp installation form the beta server.

```bash
# Debian
wget https://beta-apt.lokahost.online/lcp-install-debian.sh
# or Ubuntu
wget https://beta-apt.lokahost.online/lcp-install-ubuntu.sh
```
...
Edit `/etc/apt/sources.list.d/lokahostcp.list` and remove the `#` in front of `apt.lokahost.online`, and add a `#` in front of `beta-apt.lokahost.online`.

Once that’s done, run `apt update && apt upgrade` to rollback to the regular release.

## Reporting bugs

If you encounter a bug, please [open an issue](https://github.com/lokahostcp/lokahostcp/issues/new/choose) or [submit a Pull Request](https://github.com/lokahostcp/lokahostcp/pulls). You can also report it on our forum or our Discord server
